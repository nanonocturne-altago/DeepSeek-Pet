...
📝 **更新日志**：[update log.md](update%20log.md) —— 每次更新新增了哪些功能、修复了哪些 Bug、发布了哪些版本。
...
向音效目录（DSH 插件版：`assets/sound/`；独立版：Windows 为 exe 同级 `sound/` 文件夹，macOS 经菜单「···」按钮打开）放入成对文件：

```
小熊猫11.mp3   ← 点击声（最后一位是 1）
小熊猫12.mp3   ← 松手声（最后一位是 2）
```

- **规则只看最后一位**：文件名最后一个字符是 `1` = 点击声，`2` = 松手声；`1/2` 前面的部分就是下拉菜单里的显示名（支持中文）
- 例：`小熊猫11.mp3` + `小熊猫12.mp3` → 菜单显示「小熊猫1」；`小熊猫21.mp3` + `小熊猫22.mp3` → 菜单显示「小熊猫2」——方便给同一种音效做多种变体
- 必须成对齐全（同名 1/2 都有）才会出现在列表中；菜单每次打开时实时扫描，增删即时生效
