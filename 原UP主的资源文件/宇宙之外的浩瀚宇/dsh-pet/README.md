...
> 🔧 **本地修改说明（本机 fork，2026-08-26）**：`src/client/pet.ts` 新增 `playbackExt()` 运行时 Safari 检测——Safari 自动改用 `.mov`（HEVC-alpha），其余浏览器沿用发布期注入的扩展名。本机 link 安装（`assets/webm` 与 `assets/mov` 双素材并存）下 Chrome/Safari 同一份安装通吃；发布流程不受影响（`__PET_EXT__` 占位符保留）。
